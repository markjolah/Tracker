...
cmake -H${SRC_PATH} -B$BUILD_PATH/LinuxRelease -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DCMAKE_TOOLCHAIN_FILE=$LINUX_TOOLCHAIN_FILE -DCMAKE_BUILD_TYPE=Release -DOPT_DOC=On ${ARGS} ${@:2}
...
cmake -H${SRC_PATH} -B$BUILD_PATH/W64Release -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DCMAKE_TOOLCHAIN_FILE=$W64_TOOLCHAIN_FILE -DCMAKE_BUILD_TYPE=Release ${ARGS} ${@:2}
cmake --build $BUILD_PATH/W64Release --target install -- -j${NUM_PROCS}

if [ -n ${OPT_DEBUG} ]; then
    cmake -H${SRC_PATH} -B$BUILD_PATH/LinuxDebug -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DCMAKE_TOOLCHAIN_FILE=$LINUX_TOOLCHAIN_FILE -DCMAKE_BUILD_TYPE=Debug ${ARGS} ${@:2}
    cmake --build $BUILD_PATH/LinuxDebug --target install -- -j${NUM_PROCS}

    cmake -H${SRC_PATH} -B$BUILD_PATH/W64Debug -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DCMAKE_TOOLCHAIN_FILE=$W64_TOOLCHAIN_FILE -DCMAKE_BUILD_TYPE=Debug ${ARGS} ${@:2}
